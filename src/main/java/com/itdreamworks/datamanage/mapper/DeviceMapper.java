...
public interface DeviceMapper {
    ...
    Device modifyDeviceType(@Param("deviceSuffix") String deviceSuffix,@Param("deviceType") String deviceType, @Param("subType") String subType);
    ...
    @Insert("<script>(EnterpriseId,CustomerId,DeviceNo,DeviceSuffix,DeviceType,SubType,ImportDatetime) values "
            + "<foreach collection =\"deviceList\" item=\"device\" index=\"index\" separator =\",\"> "
            + "(#{device.enterpriseId},#{device.customerId},#{device.deviceNo},#{device.deviceSuffix},#{device.deviceType},#{device.deviceType},Now()) "
            + "</foreach ></script>")
